...
def workflow_overview(doc: Doc) -> None:
    doc.new_page()
    doc.running_head("HOW THE APPLICATION WAS BUILT"
...
def main() -> int:
    ...
    out = Path(args.out) if args.out else HERE / "Lab-1_How_The_Application_Was_Built.pdf"
    out.parent.mkdir(parents=True, exist_ok=True)
    doc = Doc(out, ASSETS)
    doc.c.setTitle("Lab 1 Companion - How the Application Was Built")

    if args.theme == "original":
        original_cover(doc, "Lab One · Companion",
