...
DB_OPERATIONS_FOOTER = (
    "Chetan Kumar M K  |  DB Operations  |  "
    "github.com/LLM-AT-SCALE/claude-agent-sdk-Labs"
)
...
def apply_theme(name: str) -> None:
    ...
    if name == "dboperations":
        # The DB_Operations palette, taken from the application's own
        # stylesheet so the guide and the software it documents read as one
        # thing: warm paper, near-black ink, a single terracotta accent.
        PAPER = HexColor("#FAF9F5")
        INK = HexColor("#211F1B")
        INK_SOFT = HexColor("#6E6C63")
        RULE = HexColor("#E8E5D8")
        GREEN = HexColor("#D97757")       # banner fill - the app's accent
        GREEN_DEEP = HexColor("#C2684A")  # accents - its hover shade
        GREEN_TINT = HexColor("#F4E1D6")  # card headers - its soft accent
        GREY_HEAD = HexColor("#F0EEE6")
        CALLOUT_BG = HexColor("#F3F1E9")
        FOOTER_TEXT = DB_OPERATIONS_FOOTER
        FOOTER_BARS = (HexColor("#D97757"), HexColor("#5E7A52"), HexColor("#2C6480"))
        USE_MASCOT = False
    elif name == "original":
